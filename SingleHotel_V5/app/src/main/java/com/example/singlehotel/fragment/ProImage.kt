...
class ProImage : BottomSheetDialogFragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.pro_image, container, false)
        method = Method(activity)
        if (method!!.isRtl) {
            view.layoutDirection = View.LAYOUT_DIRECTION_RTL
        }
        val conRemove = view.findViewById<ConstraintLayout>(R.id.con_remove_proImage)
        val conImage = view.findViewById<ConstraintLayout>(R.id.con_image_proImage)
        conRemove.setOnClickListener { v: View? ->
            val proImage = Events.ProImage("", "", false, true)
            GlobalBus.getBus().post(proImage)
            dismiss()
        }
        conImage.setOnClickListener { v: View? ->
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_CODE_CHOOSE && resultCode == Activity.RESULT_OK && data != null && data.data != null) {
            try {
                val filePath: String?
                filePath = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    method!!.getTempUploadPath(data.data)
                } else {
                    GetPath.getPath(activity, data.data)
                }
                if (filePath != null) {
                    dismiss()
                    val proImage = Events.ProImage("", filePath, true, false)
                    GlobalBus.getBus().post(proImage)
                } else {
                    method!!.alertBox(resources.getString(R.string.upload_folder_error))
                }
            } catch (e: Exception) {
                method!!.alertBox(resources.getString(R.string.upload_folder_error))
            }
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == REQUEST_CODE_PERMISSION) {
            if (grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                method!!.alertBox(resources.getString(R.string.storage_permission))
            }
        }
    }
