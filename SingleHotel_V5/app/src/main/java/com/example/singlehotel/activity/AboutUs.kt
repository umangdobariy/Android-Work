...
class AboutUs : AppCompatActivity() {
    ...
    private var cardViewEmail: MaterialCardView? = null
    private var cardViewWebsite: MaterialCardView? = null
    private var cardViewPhone: MaterialCardView? = null
    private var textViewAppName: MaterialTextView? = null
    private var textViewAppVersion: MaterialTextView? = null
    private var textViewAppAuthor: MaterialTextView? = null
    private var textViewAppContact: MaterialTextView? = null
    private var textViewAppEmail: MaterialTextView? = null
    private var textViewAppWebsite: MaterialTextView? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val linearLayout = findViewById<LinearLayout>(R.id.linearLayout_about_us)
    ...
    fun aboutUs() {
        ...
        val jsObj = Gson().toJsonTree(API(this@AboutUs)) as JsonObject
        jsObj.addProperty("method_name", "app_about")
        val apiService = ApiClient.getClient().create(ApiInterface::class.java)
        val call = apiService.getAboutUs(API.Companion.toBase64(jsObj.toString()))
        call.enqueue(object : Callback<AboutUsRP?> {
            @SuppressLint("SetJavaScriptEnabled")
            override fun onResponse(call: Call<AboutUsRP?>, response: Response<AboutUsRP?>) {
                val statusCode = response.code()
                try {
                    val aboutUsRp = response.body()!!
                    if (aboutUsRp.status == "1") {
                        textViewAppName!!.text = aboutUsRp.app_name
                        Glide.with(this@AboutUs).load(aboutUsRp.app_logo)
                            .placeholder(R.drawable.placeholder_landscape)
                            .into(imageView!!)
                        textViewAppVersion!!.text = aboutUsRp.app_version
                        textViewAppAuthor!!.text = aboutUsRp.app_author
                        textViewAppContact!!.text = aboutUsRp.app_contact
                        textViewAppEmail!!.text = aboutUsRp.app_email
                        textViewAppWebsite!!.text = aboutUsRp.app_website
                        ...
                        val text = ("<html dir=" + method!!.isWebViewTextRtl + "><head>"
                                ...
                                + "</style></head>"
                                + "<body>"
                                + aboutUsRp.app_description
                                + "</body></html>")
                        webView!!.loadDataWithBaseURL(null, text, mimeType, encoding, null)
                        conMain!!.visibility = View.VISIBLE
                        cardViewEmail!!.setOnClickListener { v: View? ->
                            try {
                                ...
                                emailIntent.data = Uri.parse("mailto:?subject=")
                                startActivity(emailIntent)
                            } catch (ex: ActivityNotFoundException) {
                                method!!.alertBox(resources.getString(R.string.wrong))
                            }
                        }
                        cardViewWebsite!!.setOnClickListener { v: View? ->
                            try {
                                var url = aboutUsRp.app_website
                        ...
                        cardViewPhone!!.setOnClickListener { v: View? ->
                            try {
                                val callIntent = Intent(Intent.ACTION_DIAL)
                                callIntent.data = Uri.parse("tel:" + aboutUsRp.app_contact)
                        ...
                        method!!.alertBox(aboutUsRp.message)
                    }
                } catch (e: Exception) {
            ...
            override fun onFailure(call: Call<AboutUsRP?>, t: Throwable) {
