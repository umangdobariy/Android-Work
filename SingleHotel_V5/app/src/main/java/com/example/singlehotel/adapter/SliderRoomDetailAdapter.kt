...
class SliderRoomDetailAdapter(
    private val activity: Activity,
    private val type: String,
    private val roomSliders: List<RoomSlider>
) : PagerAdapter() {
    private val inflater: LayoutInflater
    @SuppressLint("SetTextI18n")
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        val imageLayout = inflater.inflate(R.layout.slider_adapter, container, false)!!
        val imageView = imageLayout.findViewById<ImageView>(R.id.imageView_slider_adapter)
        imageLayout.tag = EnchantedViewPager.Companion.ENCHANTED_VIEWPAGER_POSITION + position
        Glide.with(activity).load(roomSliders[position].image_name)
