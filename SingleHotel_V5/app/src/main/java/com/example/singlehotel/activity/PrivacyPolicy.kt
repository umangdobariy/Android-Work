...
class PrivacyPolicy : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        method = Method(this@PrivacyPolicy)
        method!!.forceRTLIfSupported()
        toolbar = findViewById(R.id.toolbar_privacy_policy)
        toolbar.setTitle(resources.getString(R.string.privacy_policy))
        ...
        val linearLayout = findViewById<LinearLayout>(R.id.linearLayout_privacy_policy)
    ...
    fun privacyPolicy() {
        ...
        val jsObj = Gson().toJsonTree(API(this@PrivacyPolicy)) as JsonObject
        jsObj.addProperty("method_name", "app_privacy_policy")
        val apiService = ApiClient.getClient().create(ApiInterface::class.java)
        val call = apiService.getPrivacyPolicy(API.Companion.toBase64(jsObj.toString()))
        call.enqueue(object : Callback<PrivacyPolicyRP?> {
            @SuppressLint("SetJavaScriptEnabled")
            override fun onResponse(
                call: Call<PrivacyPolicyRP?>,
                response: Response<PrivacyPolicyRP?>
            ) {
                val statusCode = response.code()
                try {
                    val privacyPolicyRP = response.body()!!
                    if (privacyPolicyRP.status == "1") {
                        ...
                        val text = ("<html dir=" + method!!.isWebViewTextRtl + "><head>"
                                ...
                                + "</style></head>"
                                + "<body>"
                                + privacyPolicyRP.privacy_policy
                                + "</body></html>")
                        webView!!.loadDataWithBaseURL(null, text, mimeType, encoding, null)
                    } else {
                        conNoData!!.visibility = View.VISIBLE
                        method!!.alertBox(privacyPolicyRP.message)
                    }
                } catch (e: Exception) {
            ...
            override fun onFailure(call: Call<PrivacyPolicyRP?>, t: Throwable) {
