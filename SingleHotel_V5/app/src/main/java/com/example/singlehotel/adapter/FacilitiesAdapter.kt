...
import com.example.singlehotel.itemimport.FacilitiesList
...
class FacilitiesAdapter(
    ...
    private val facilitiesLists: List<FacilitiesList>
) : RecyclerView.Adapter<FacilitiesAdapter.ViewHolder>() {
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): FacilitiesAdapter.ViewHolder {
        val view = LayoutInflater.from(activity).inflate(R.layout.facilities_adapter, parent, false)
        return FacilitiesAdapter.ViewHolder(view)
    }

    @SuppressLint("SetTextI18n")
    override fun onBindViewHolder(holder: FacilitiesAdapter.ViewHolder, position: Int) {
        holder.textViewName.text = facilitiesLists[position].facilities.trim { it <= ' ' }
