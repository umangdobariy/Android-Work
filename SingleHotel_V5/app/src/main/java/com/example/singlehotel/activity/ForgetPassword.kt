...
class ForgetPassword : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        method = Method(this@ForgetPassword)
        method!!.forceRTLIfSupported()
        progressDialog = ProgressDialog(this@ForgetPassword)
        imm = getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager
        window.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_HIDDEN)
        val toolbar = findViewById<MaterialToolbar>(R.id.toolbar_fp)
        toolbar.title = resources.getString(R.string.forget_password)
        setSupportActionBar(toolbar)
        supportActionBar!!.setDisplayHomeAsUpEnabled(true)
        supportActionBar!!.setDisplayShowHomeEnabled(true)
        editText = findViewById(R.id.editText_fp)
        val button = findViewById<MaterialButton>(R.id.button_fp)
        button.setOnClickListener { v: View? ->
            val stringFp = editText.getText().toString()
            editText.setError(null)
            if (!isValidMail(stringFp) || stringFp.isEmpty()) {
                editText.requestFocus()
                editText.setError(resources.getString(R.string.please_enter_email))
            } else {
                editText.clearFocus()
    ...
    fun forgetPassword(sendEmail: String?) {
        progressDialog!!.show()
        progressDialog!!.setMessage(resources.getString(R.string.loading))
        progressDialog!!.setCancelable(false)
        val jsObj = Gson().toJsonTree(API(this@ForgetPassword)) as JsonObject
        jsObj.addProperty("user_email", sendEmail)
        jsObj.addProperty("method_name", "forgot_pass")
        val apiService = ApiClient.getClient().create(ApiInterface::class.java)
        val call = apiService.getForgotPass(API.Companion.toBase64(jsObj.toString()))
