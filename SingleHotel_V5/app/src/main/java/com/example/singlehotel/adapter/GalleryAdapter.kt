package com.example.singlehotel.adapterimport
...
class GalleryAdapter(
    ...
    private val galleryLists: List<GalleryList?>,
    onClick: OnClick?
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == VIEW_TYPE_ITEM) {
            val view =
                LayoutInflater.from(activity).inflate(R.layout.gallery_adapter, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder?, position: Int) {
        if (holder!!.itemViewType == VIEW_TYPE_ITEM) {
            val viewHolder = holder as GalleryAdapter.ViewHolder?
            viewHolder!!.imageView.layoutParams =
                ConstraintLayout.LayoutParams(columnWidth, columnWidth)
            viewHolder.view.layoutParams = ConstraintLayout.LayoutParams(columnWidth, columnWidth)
            Glide.with(activity).load(galleryLists[position]!!.category_image_thumb)
                .placeholder(R.drawable.placeholder_portable)
                .into(viewHolder.imageView)
            viewHolder.textView.text = galleryLists[position]!!.category_name
            viewHolder.constraintLayout.setOnClickListener { v: View? ->
                method.onClickAd(
                    position, type, galleryLists[position]!!
                        .cid, galleryLists[position]!!.category_name
                )
            }
        } else if (holder.itemViewType == VIEW_TYPE_ADS) {
            val adOption = holder as GalleryAdapter.AdOption?
    ...
    fun hideHeader() {
        GalleryAdapter.ProgressViewHolder.Companion.progressBar.setVisibility(View.GONE)
    }
    ...
    override fun getItemViewType(position: Int): Int {
        return if (galleryLists.size == position) {
            VIEW_TYPE_LOADING
        } else if (galleryLists[position] == null) {
            VIEW_TYPE_ADS
        } else {
            VIEW_TYPE_ITEM
        }
    ...
    class ProgressViewHolder(v: View) : RecyclerView.ViewHolder(v) {
        ...
        init {
            GalleryAdapter.ProgressViewHolder.Companion.progressBar =
                v.findViewById<ProgressBar>(R.id.progressBar_loading)
        }
    }

    inner class AdOption(itemView: View) : RecyclerView.ViewHolder(itemView) {
        private val card_view: CardView
        private val rl_native_ad: RelativeLayout
        private val isAdRequested = false
    ...
    init {
        ...
        val padding = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2f, r.displayMetrics)
        columnWidth = ((method.screenWidth - (4 + 2) * padding) / 2).toInt()
    }
}