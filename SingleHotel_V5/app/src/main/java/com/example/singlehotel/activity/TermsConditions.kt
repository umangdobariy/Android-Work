...
class TermsConditions : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        method = Method(this@TermsConditions)
        method!!.forceRTLIfSupported()
        val toolbar = findViewById<MaterialToolbar>(R.id.toolbar_terms)
        toolbar.title = resources.getString(R.string.terms_condition)
        ...
        val linearLayout = findViewById<LinearLayout>(R.id.linearLayout_terms)
    ...
    val terms: Unit
        get() {
            ...
            val jsObj = Gson().toJsonTree(API(this@TermsConditions)) as JsonObject
            jsObj.addProperty("method_name", "app_terms_conditions")
            val apiService = ApiClient.getClient().create(ApiInterface::class.java)
            val call = apiService.getTermsCondition(API.Companion.toBase64(jsObj.toString()))
            call.enqueue(object : Callback<TermsConditionsRP?> {
                @SuppressLint("SetJavaScriptEnabled")
                override fun onResponse(
                    call: Call<TermsConditionsRP?>,
                    response: Response<TermsConditionsRP?>
                ) {
                    try {
                        val termsConditionsRP = response.body()!!
                        if (termsConditionsRP.status == "1") {
                            ...
                            val text = ("<html dir=" + method!!.isWebViewTextRtl + "><head>"
                                    ...
                                    + "</style></head>"
                                    + "<body>"
                                    + termsConditionsRP.app_terms_conditions
                                    + "</body></html>")
                            webView!!.loadDataWithBaseURL(null, text, mimeType, encoding, null)
                        } else {
                            conNoData!!.visibility = View.VISIBLE
                            method!!.alertBox(termsConditionsRP.message)
                        }
                    } catch (e: Exception) {
                ...
                override fun onFailure(call: Call<TermsConditionsRP?>, t: Throwable) {
                    // Log error here since request failed
