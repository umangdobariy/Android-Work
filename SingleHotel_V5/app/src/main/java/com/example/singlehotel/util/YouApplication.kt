...
class YouApplication : Application() {
    override fun onCreate() {
        super.onCreate()
        MobileAds.initialize(this@YouApplication) { initializationStatus: InitializationStatus? -> }

        // OneSignal Initialization
        OneSignal.initWithContext(this)
        OneSignal.setAppId("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
        OneSignal.setNotificationOpenedHandler(NotificationExtenderExample())
        ViewPump.init(
            ViewPump.builder()
                .addInterceptor(
    ...
    internal inner class NotificationExtenderExample : OSNotificationOpenedHandler {
        override fun notificationOpened(result: OSNotificationOpenedResult) {
            try {
                val id = result.notification.additionalData.getString("id")
                val roomTitle = result.notification.additionalData.getString("room_title")
                val url = result.notification.additionalData.getString("external_link")
                val intent: Intent
                if (id == "0" && url != "false" && !url.trim { it <= ' ' }.isEmpty()) {
                    intent = Intent(Intent.ACTION_VIEW)
                    intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
                    intent.data = Uri.parse(url)
                } else {
                    intent = Intent(this@YouApplication, SplashScreen::class.java)
                    intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
