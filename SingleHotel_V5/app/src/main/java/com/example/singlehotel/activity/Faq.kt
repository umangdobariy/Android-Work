...
class Faq : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        method = Method(this@Faq)
        method!!.forceRTLIfSupported()
        val toolbar = findViewById<MaterialToolbar>(R.id.toolbar_faq)
        toolbar.title = resources.getString(R.string.faq)
        ...
        val linearLayout = findViewById<LinearLayout>(R.id.linearLayout_faq)
    ...
    fun faq() {
        ...
        val jsObj = Gson().toJsonTree(API(this@Faq)) as JsonObject
        jsObj.addProperty("method_name", "app_faq")
        val apiService = ApiClient.getClient().create(ApiInterface::class.java)
        val call = apiService.getFaq(API.Companion.toBase64(jsObj.toString()))
        call.enqueue(object : Callback<FaqRP?> {
            @SuppressLint("SetJavaScriptEnabled")
            override fun onResponse(call: Call<FaqRP?>, response: Response<FaqRP?>) {
                val statusCode = response.code()
                try {
                    val faqRP = response.body()!!
                    if (faqRP.status == "1") {
            ...
            override fun onFailure(call: Call<FaqRP?>, t: Throwable) {
