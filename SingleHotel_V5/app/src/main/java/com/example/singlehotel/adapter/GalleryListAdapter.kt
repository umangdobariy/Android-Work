...
class GalleryListAdapter(
    ...
    private val galleryDetailLists: List<GalleryDetailList>,
    onClick: OnClick?
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (viewType == VIEW_TYPE_ITEM) {
            val view = LayoutInflater.from(activity)
                .inflate(R.layout.gallery_recyclerview_adapter, parent, false)
            return GalleryListAdapter.ViewHolder(view)
        } else if (viewType == VIEW_TYPE_LOADING) {
            val v = LayoutInflater.from(activity)
                .inflate(R.layout.layout_horizontal_loading_item, parent, false)
            return GalleryListAdapter.ProgressViewHolder(v)
        }
        return null
    }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder?, position: Int) {
        if (holder!!.itemViewType == VIEW_TYPE_ITEM) {
            val viewHolder = holder as GalleryListAdapter.ViewHolder?
            Glide.with(activity).load(galleryDetailLists[position].wallpaper_image_thumb)
                .placeholder(R.drawable.placeholder_portable)
                .into(viewHolder!!.imageView)
            viewHolder.imageView.setOnClickListener { v: View? ->
                method.onClickAd(
                    position,
                    type,
                    galleryDetailLists[position].id,
                    ""
                )
            }
        }
    ...
    fun hideHeader() {
        GalleryListAdapter.ProgressViewHolder.Companion.progressBar.setVisibility(View.GONE)
    }
    ...
    override fun getItemViewType(position: Int): Int {
        return if (isHeader(position)) VIEW_TYPE_LOADING else VIEW_TYPE_ITEM
    }
    ...
    class ProgressViewHolder(v: View) : RecyclerView.ViewHolder(v) {
        ...
        init {
            GalleryListAdapter.ProgressViewHolder.Companion.progressBar =
                v.findViewById<ProgressBar>(
                    R.id.progressBar_horizontal_loading
                )
        }
    }
