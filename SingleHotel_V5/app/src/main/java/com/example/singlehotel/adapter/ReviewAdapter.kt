...
class ReviewAdapter(
    ...
    private val reviewLists: List<ReviewList>
) : RecyclerView.Adapter<ReviewAdapter.ViewHolder>() {
    private val lastPosition = -1
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ReviewAdapter.ViewHolder {
        val view = LayoutInflater.from(activity).inflate(R.layout.review_adapter, parent, false)
        return ReviewAdapter.ViewHolder(view)
    }

    override fun onBindViewHolder(holder: ReviewAdapter.ViewHolder, position: Int) {
        if (reviewLists[position].user_image != "") {
            Glide.with(activity).load(reviewLists[position].user_image)
        ...
        holder.imageView.setOnClickListener { v: View? ->
            activity.startActivity(
                Intent(
                    ...
                    .putExtra("path", reviewLists[position].user_image)
            )
        }
        holder.textViewName.text = reviewLists[position].user_name
        holder.textViewDate.text = reviewLists[position].dt_rate
        holder.textViewMsg.text = reviewLists[position].message
        holder.ratingView.rating = reviewLists[position].rate.toFloat()
        if (position > lastPosition) {
            if (position % 2 == 0) {
                holder.con.setBackgroundColor(activity.resources.getColor(R.color.background_item_one_review_adapter))
            } else {
                holder.con.setBackgroundColor(activity.resources.getColor(R.color.background_item_two_review_adapter))
            }
        }
    }
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        private val con: ConstraintLayout
        private val ratingView: RatingView
        private val imageView: CircleImageView
        private val textViewName: MaterialTextView
        private val textViewDate: MaterialTextView
        private val textViewMsg: MaterialTextView
